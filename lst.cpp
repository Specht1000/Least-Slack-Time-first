// Nome: Guilherme Martins Specht
// Trabalho 1 da disciplina Sistemas de Tempo Real do Prof. Roland Teodorowitsch no qual consiste
// em desenvolver o escalonador de tarefas Least Slack Time first (LST).
// O input para este código é gerado a partir do comando ./lst < input.txt
...
struct Tarefa{
    char letra;      // Letra correspondente a tarefa.
...
int tempo_slack(const Tarefa& Tarefa, int tempo_atual){
    ...
    tempo_ate_deadline = Tarefa.inicio + Tarefa.deadline - tempo_atual; // Calculo do tempo até o fim do deadline.
    slack = tempo_ate_deadline - Tarefa.tempo_falta; // Calculo do slack.
    return slack;
}
        ...
        for(tempo_atual = 0; tempo_atual < tempo_execucao; tempo_atual++){
            ...
            for(Tarefa& Tarefa : Tarefas){
                if (Tarefa.tempo_falta > 0 && tempo_slack(Tarefa, tempo_atual) < min_slack){ // Verifica se ainda tem tempo de execução restante e se o slack da tarefa atual é menor do que o min_slack da tarefa anterior.
                    ...
                    proxima_tarefa = &Tarefa; // next_tarefa aponta para a tarefa atual.
                }
            }

            if(proxima_tarefa){
                escalonamento += proxima_tarefa->letra; // A tarefa com menor slack é escalonada.
                ...
                if(proxima_tarefa->tempo_falta == 0){
                    foi_preemptado = false; // Se a tarefa terminou, não há preempção.
                }
                else{
